...
# 数値データ列を明示的に指定して抽出
numerical_data = data.iloc[:, 2:]  # "グループ"以降の数値列を取得
numerical_data = numerical_data.apply(pd.to_numeric, errors='coerce')  # 数値変換できない値をNaNに

# NaNを含む行を削除
valid_data = numerical_data.dropna()

# 有効なデータが存在しない場合のエラーハンドリング
if valid_data.shape[0] == 0:
    print("有効なデータがありません。データを確認してください。")
    exit()
...
# 有効データに対応するグループ情報を取得
valid_groups = data.loc[valid_data.index, 'グループ']

# グループごとのカラー設定
unique_groups = valid_groups.unique()
group_colors = plt.colormaps['tab10'](np.linspace(0, 1, len(unique_groups)))  # カラーマップの取得

# PC1 vs PC2 のプロット
plt.figure(figsize=(8, 6))
for i, group in enumerate(unique_groups):
    group_points = pca_result_3d[valid_groups == group][:, :2]  # グループごとのPC1とPC2
    plt.scatter(
...
for i, group in enumerate(unique_groups):
    # valid_groupsを用いてpca_result_3dをフィルタリング
    group_points = pca_result_3d[valid_groups == group][:, [0, 2]]  # グループごとのPC1とPC3
    plt.scatter(
        group_points[:, 0], group_points[:, 1],
        label=f"Group {group}",
        color=group_colors[i],
...
for i, group in enumerate(unique_groups):
    # valid_groupsを使用してグループごとのデータを取得
    group_points = pca_result_3d[valid_groups == group][:, [1, 2]]  # グループごとのPC2とPC3
    plt.scatter(
        group_points[:, 0], group_points[:, 1],
        label=f"Group {group}",
        color=group_colors[i],  # 修正: 配列としてインデックスでアクセス
        alpha=0.6, s=50
    )
