'''
This module is for 2nd term experiment data
'''
...
def cal_saccade_count_ratio(df):
...
def main():
    ...
    # 特徴量の計算
    time_range = cal_time_range(df)
    avg_velocity = cal_avg_velocity(df)
    transition_count = cal_transition_count(df)
    saccade_count_ratio = cal_saccade_count_ratio(df)
    avg_saccade_amplitude = cal_avg_saccade_amp(df)
    max_saccade_amplitude = cal_max_saccade_amp(df)
    gaze_dispersion = cal_gaze_dispersion(df)
    fractal_dimension = cal_fractal_dimension(df)
    avg_path_linearity = cal_avg_path_linearity(df)
